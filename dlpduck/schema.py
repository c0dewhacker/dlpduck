...
it is already masked — there is nothing in it that privacy or a
takedown request would ever require deleting. A job accumulates one row
per *assessment*: the first at ingest, one more each time
...
and append-only. Purge does not need to rewrite an index
...
joined string has already destroyed. Reprocessing's "rules" mode
...
INDEX_SCHEMA = pa.schema(
    [
        ...
        ("release_pending", pa.bool_()),  # de-escalated logically, PDF not yet moved
        ...
        ("metadata", pa.string()),  # JSON, allowlisted keys only
